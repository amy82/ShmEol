...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	if (!pState->m_bNeedTerm && !AfxOleInit())
		//theApp.MessageView(MESSAGE_VIEW,LOG_ALL,1,"OLE initialization failed. Make sure that the OLE libraries are the correct version",300);
...
BOOL CGridCtrl::SetItemText(int nRow, int nCol, double dVal, int iDecimal)
{
	if (GetVirtualMode())
		return FALSE;
	...
	CString m_stValue = _T("");
	CString m_stTemp = _T("");
	CString m_stDecimal = _T(""), m_stInteger = _T("");
	m_stValue.Format(_T("%lf"), dVal);

	int iValLenght = m_stValue.GetLength();
	int iCheckDecimal = m_stValue.Find('.', 0);

	m_stInteger = m_stValue.Mid(0, iCheckDecimal);
	m_stDecimal = m_stValue.Mid(iCheckDecimal, iValLenght- (iCheckDecimal) );

	
	int iDecimalLenght = m_stDecimal.GetLength();
	
	for(int i=iDecimalLenght-1; i>0; i--)
	{
		if(m_stDecimal.GetAt(i) == '.' )
		{
			m_stTemp = m_stDecimal.Mid(0, i+2);
			break;
		}
		...
		if( iDecimal != 0 )
		{
			if( i < iDecimal+1 )
			{
				m_stTemp = m_stDecimal.Mid(0, i+1);
				break;		
			}
		}
		else
		{
			if(m_stDecimal.GetAt(i) != '0' )
			{
				m_stTemp = m_stDecimal.Mid(0, i+1);
				break;		
			}
	...
	m_stValue = m_stInteger + m_stTemp;
	pCell->SetText(m_stValue);
...
BOOL CGridCtrl::SetItemText(int nRow, int nCol, int iVal)
{
	if (GetVirtualMode())
		return FALSE;
...
BOOL CGridCtrl::SetItemFormatRow(int nRow, UINT nFormat)
{
	if (GetVirtualMode())
		return FALSE;
	...
	{
		CGridCellBase* pCell = GetCell(nRow, iCol);
		ASSERT(pCell);
		if (!pCell)
			return FALSE;
...
BOOL CGridCtrl::SetItemFormatCol(int nCol, UINT nFormat)
{
	if (GetVirtualMode())
		return FALSE;
	...
	{
		CGridCellBase* pCell = GetCell(iRow, nCol);
		ASSERT(pCell);
		if (!pCell)
			return FALSE;
...
COLORREF CGridCtrl::GetItemBkColor(int nRow, int nCol) const
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
COLORREF CGridCtrl::GetItemFgColor(int nRow, int nCol) const
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CGridCtrl::SetRowHeightSelect(int iSRow, int iERow, int height)
{
	ASSERT(iSRow >= 0 && iSRow < m_nRows && height >= 0);
	ASSERT(iERow >= 0 && iERow < m_nRows && height >= 0);

	for( int nRow = iSRow; nRow < iERow+1; nRow++)
	{
		if (nRow < 0 || nRow >= m_nRows || height < 0)
			return FALSE;
...
BOOL CGridCtrl::SetColumnWidthSelect(int iSCol, int iECol, int iwidth)
{
	ASSERT(iSCol >= 0 && iSCol < m_nCols && iwidth >= 0);
	ASSERT(iECol >= 0 && iECol < m_nCols && iwidth >= 0);

	for( int nCol = iSCol; nCol < iECol+1; nCol++)
	{
		if (nCol < 0 || nCol >= m_nCols || iwidth < 0)
			return FALSE;

		m_arColWidths[m_arColOrder[nCol]] = iwidth;	
	}
	ResetScrollBars();
...
BOOL CGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
	...
	{
		//theApp.MessageView(MESSAGE_VIEW,LOG_ALL,1,"Unable to save grid list",300);
...
BOOL CGridCtrl::Load(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
	...
	CATCH (CFileException, e)
	{
		//theApp.MessageView(MESSAGE_VIEW,LOG_ALL,1,"Unable to load grid data",300);
