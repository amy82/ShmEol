...
{
	std::string strAlgorithm; ///< DOT, CROSSDOT, ...
...
typedef struct _TFiducialMarkInfoN
{
	...
	std::string strAlgorithm; /// "[Item][index]" 꼴의 문자열
	                          /// [Item] - F: FOV, D: Distortion, T: Tilt, R: Rotation
	                          /// [index] - 0, 1, 2... (각 열거형에 따름)
	                          /// ex> "F3": FOV_METHOD_SHM_CORNER
	std::vector<double> vParam; /// 연산에 추가적으로 사용되는 값들을 전달하기 위한 벡터
