...
#include <LibACMISCodec\LibACMISCodecCommon.h>
...
CLaonGrabberWrapper::CLaonGrabberWrapper()
{
	...
	vYuvChartBuffer = NULL;	//CLaonGrabberWrapper
	vRawChartBuffer = NULL;
	
	vRawBuffer_3000k = NULL;
	...
	_stprintf_s(strRawFileName[YUV_CHART], SIZE_OF_1K, _T(""));
	_stprintf_s(strRawFileName[RAW_CHART], SIZE_OF_1K, _T(""));
	_stprintf_s(strRawFileName[RAW_OC], SIZE_OF_1K, _T(""));
...
CLaonGrabberWrapper::~CLaonGrabberWrapper()
{
	...
	if (vYuvChartBuffer)		//~CLaonGrabberWrapper()
	{
		delete vYuvChartBuffer;
	}
	vYuvChartBuffer = NULL;

	if (vRawChartBuffer)
	{
		delete vRawChartBuffer;
	}
	vRawChartBuffer = NULL;
	
	if (vRawBuffer_3000k)
	{
		delete vRawBuffer_3000k;
	}
	vRawBuffer_3000k = NULL;
...
//
//-----------------------------------------------------------------------------
void CLaonGrabberWrapper::rawReSet(int nMode)
{
	int i = 0;

	int nCurrentSize = m_pBoard->GetFrameRawSize();
	int bmpsize = m_pBoard->GetFrameBMPSize();			//6220800
	...
	m_pFrameRawBuffer = new BYTE[nCurrentSize];
	memset(m_pFrameRawBuffer, 0, nCurrentSize);
	...
	if (nMode == 0)		//RAW
	{
		for (i = 0; i < 10; i++)
		{
			...
			pSumBuffer[i] = new BYTE[N_RAW_SIZE];
			memset(pSumBuffer[i], 0, N_RAW_SIZE);
		}

		if (vRawChartBuffer)					//rawReSet 0
		{
			delete vRawChartBuffer;
		}
		if (vRawBuffer_3000k)
		...
		vRawChartBuffer = new BYTE[N_RAW_SIZE];
		vRawBuffer_3000k = new BYTE[N_RAW_SIZE];

		memset(vRawChartBuffer, 0, N_RAW_SIZE);
		memset(vRawBuffer_3000k, 0, N_RAW_SIZE);


	}
	else if (nMode == 1)		//YUV
	{
		if (vYuvChartBuffer)					//rawReSet 1
		{
			delete vYuvChartBuffer;
		}

		vYuvChartBuffer = new BYTE[N_YUV_SIZE];
		memset(vYuvChartBuffer, 0, N_YUV_SIZE);
	}
	else
	{
		...
		//
		//
		if (vYuvChartBuffer)					//rawReSet else
		{
			delete vYuvChartBuffer;
		}
...
bool CLaonGrabberWrapper::OpenDevice()
{
	...
	nErrorCode = m_pBoard->OpenBoard(m_cBoardIndex);
...
void EncodeCallback(const JXL_ENC_RSP* rsp)
{
	char szDebug[MAX_PATH] = { 0, };

	if (rsp->enErrorCode == ERROR_ENC_SUCCESS)
	{
		sprintf_s(szDebug, "%s - Encode Success\r\n", rsp->szFileName);
		OutputDebugStringA(szDebug);
	}
	else
	{
		sprintf_s(szDebug, "%s - Encode Fail(Error Code:%d)\r\n", rsp->szFileName, rsp->enErrorCode);
		OutputDebugStringA(szDebug);
	}
}
void DecodeCallback(const JXL_DEC_RSP* rsp)
{
	char szDebug[MAX_PATH] = { 0, };

	if (rsp->enErrorCode == ERROR_DEC_SUCCESS)
	{
		sprintf_s(szDebug, "%s - Decode Success\r\n", rsp->szFileName);
		OutputDebugStringA(szDebug);
	}
	else
	{
		sprintf_s(szDebug, "%s - Decode Fail(Error Code:%d)\r\n", rsp->szFileName, rsp->enErrorCode);
		OutputDebugStringA(szDebug);
	}
}
bool CLaonGrabberWrapper::ImageDecode(const char* pszFileName, const char* pszFolderName)	//jxl -> raw ¾ÐÃà ÇØÁ¦
{
	DEC_JXL_Init(DecodeCallback);

	JXL_DEC_RSP* pResult = new JXL_DEC_RSP();
	memset(pResult, 0, sizeof(JXL_DEC_RSP));

	if (DEC_JXL_LOAD_IMAGE_FILE(pszFileName, pResult) != 0)
	{
		for (int i = 0; i < MAX_DECODE_FRAMES; i++)
		{
			if (pResult->tagImageData[i].pRGB)
			{
				delete[] pResult->tagImageData[i].pRGB;
				pResult->tagImageData[i].pRGB = NULL;
			}
			if (pResult->tagImageData[i].pPacked)
			{
				delete[] pResult->tagImageData[i].pPacked;
				pResult->tagImageData[i].pPacked = NULL;
			}
		}
		if (pResult)
		{
	...
	for (int i = 0; i < MAX_DECODE_FRAMES; i++)
	{
		if (pResult->tagImageData[i].nWidth < 1 || pResult->tagImageData[i].nHeight < 1) { break; }

		DECODE_DATA* ptagData = &pResult->tagImageData[i];
		int nFileExt = ptagData->tFileInfo.nFileExt;
		std::string strFrameName = ptagData->szFrameName;
		size_t index = strFrameName.find_last_of('\\') == std::string::npos ? 0 : strFrameName.find_last_of('\\');
		std::string strNameOnly = strFrameName.substr(index);
		char szSrcFile[512] = { 0, };
		if (nFileExt == FileExt_PNG)
		{
			sprintf_s(szSrcFile, "%s\\%s_decoded.png", pszFolderName, strNameOnly.c_str());
		}
		else if (nFileExt == FileExt_BMP)
		{
			sprintf_s(szSrcFile, "%s\\%s_decoded.bmp", pszFolderName, strNameOnly.c_str());
		}
		else if (nFileExt == FileExt_JPG)
		{
			sprintf_s(szSrcFile, "%s\\%s_decoded.jpg", pszFolderName, strNameOnly.c_str());
		}
		else // Raw
		{
			sprintf_s(szSrcFile, "%s\\%s_decoded.raw", pszFolderName, strNameOnly.c_str());
		}
		...
		do
		{
			if (nFileExt != FileExt_PNG && nFileExt != FileExt_BMP) // JPG or Raw
			{
				FILE* file = NULL;
				int nImageDataSize = ptagData->tFileInfo.nFileSize;

				fopen_s(&file, szSrcFile, "wb");
				if (!file)
				{
					//strMessage.Format(_T("Could not open %s for writing"), szSrcFile);

					_stprintf_s(szLog, SIZE_OF_1K, _T("Could not open %s for writing"), szSrcFile);
					AddLog(szLog, 0, m_nUnit);
					break;
				}
				if (fwrite(ptagData->pPacked, sizeof(uint8_t), nImageDataSize, file) != nImageDataSize)
				{
					//strMessage.Format(_T("Could not write bytes to %s"), szSrcFile);
					_stprintf_s(szLog, SIZE_OF_1K, _T("Could not write bytes to %s"), szSrcFile);
					AddLog(szLog, 0, m_nUnit);
					fclose(file);
					break;
				}
				if (fclose(file) != 0)
				{
					//strMessage.Format(_T("Could not close %s"), szSrcFile);
					_stprintf_s(szLog, SIZE_OF_1K, _T("Could not close %s"), szSrcFile);
					AddLog(szLog, 0, m_nUnit);
					break;
				}

				//strMessage.Format(_T("Raw Write:[%s]"), szSrcFile);
				_stprintf_s(szLog, SIZE_OF_1K, _T("Raw Write:[%s]"), szSrcFile);
				AddLog(szLog, 0, m_nUnit);
			}
			else // BMP or PNG
			{
				//ACMISSoftISP::xSaveImage(szSrcFile, ptagData->pPacked, ptagData->nWidth, ptagData->nHeight, ptagData->nColorByte == 2 ? CV_16UC1 : CV_8UC3);
			}
		} while (0);
	}
...
bool CLaonGrabberWrapper::ImageEncode()		//raw -> jxl ·Î ¾ÐÃà
{
	int i = 0;
	std::vector<IMAGE_DATA*> vImageDatas;
	std::vector<std::string> vImageFiles;
	std::string strPath;
	TCHAR szFileName[SIZE_OF_1K];
	TCHAR szLog[SIZE_OF_1K];
...
//#endif
	...
	//_stprintf_s(szFileName, SIZE_OF_1K, _T("%s_%s.raw"), strRawFileName[YUV_CHART], m_szModel);			//È®ÀåÀÚ »èÁ¦
	_stprintf_s(szFileName, SIZE_OF_1K, _T("%s_%s"), strRawFileName[YUV_CHART], m_szModel);
	strPath = std::string(szFileName);
	vImageFiles.push_back(strPath);		//¿©±â ³Ö´Â ¼ø¼­°¡ »ý¼ºµÇ´Â ÆÄÀÏ¸íÀÌ´Ù.
	//
	_stprintf_s(szFileName, SIZE_OF_1K, _T("%s_%s"), strRawFileName[RAW_CHART], m_szModel);
	strPath = std::string(szFileName);
	vImageFiles.push_back(strPath);
	//
	_stprintf_s(szFileName, SIZE_OF_1K, _T("%s_%s"), strRawFileName[RAW_OC], m_szModel);
	strPath = std::string(szFileName);
	vImageFiles.push_back(strPath);


	_stprintf_s(szLog, SIZE_OF_1K, _T("ENC_JXL_SetEncEffort : %d"), g_clModelData[m_nUnit].m_nJxlEffort); 
	AddLog(szLog, 0, m_nUnit);


	ENC_JXL_Init(EncodeCallback);
	ENC_JXL_SetEncEffort(g_clModelData[m_nUnit].m_nJxlEffort);// 6); // Effort : 1 ~ 10. ¿ÜºÎ¿¡¼­ ¼³Á¤ º¯°æÇÒ ¼ö ÀÖµµ·Ï ±¸Çö ÇÊ¿ä

	for (int i = 0; i < (int)vImageFiles.size(); i++)
	{
		IMAGE_DATA* ptagImageData = new IMAGE_DATA();
		strcpy_s(ptagImageData->szFrameName, vImageFiles[i].c_str()); // °¢ Raw Image ÆÄÀÏ¸í



		//if (i == 0) // YUV Image
		if (vImageFiles[i].find("Yuv") != std::string::npos) // YUV Image
		{
			ptagImageData->tFileInfo.nDataFormat = 0; // DATAFORMAT_YUV
			ptagImageData->tFileInfo.nOutMode = 3; // OUTMODE_YUV422_CbYCrY
			ptagImageData->tFileInfo.nSensorType = 5; // SENSORTYPE_OTHER
			ptagImageData->tFileInfo.nDemosaic = 0; // DEMOSAICMETHOD_NONE
			ptagImageData->tFileInfo.nSplitCFA = 1; // on
			ptagImageData->tFileInfo.nFileSize = N_YUV_SIZE;
		}
		else // Bayer Raw Image
		{
			ptagImageData->tFileInfo.nDataFormat = 4; // DATAFORMAT_BAYER_12BIT
			ptagImageData->tFileInfo.nOutMode = 2; // OUTMODE_BAYER_RGGB
			ptagImageData->tFileInfo.nSensorType = 0; // SENSORTYPE_RGGB
			ptagImageData->tFileInfo.nDemosaic = 1; // DEMOSAICMETHOD_GRADIENT
			ptagImageData->tFileInfo.nSplitCFA = 1; // on
			ptagImageData->tFileInfo.nFileSize = N_RAW_SIZE;
		}

		ptagImageData->tFileInfo.nEmbeddedSizeFront = 0;
		ptagImageData->tFileInfo.nEmbeddedSizeRear = 0;
		ptagImageData->tFileInfo.nFileExt = FileExt_RAW;

		ptagImageData->nWidth = 1920;
		ptagImageData->nHeight = 1080;
		ptagImageData->nStride = 1920;
		ptagImageData->nColorType = 0;
		ptagImageData->nBitDepth = 16;

		ptagImageData->pImageData = new uint8_t[ptagImageData->tFileInfo.nFileSize];

		//memcpy(ptagImageData->pImageData, pBuffer, buffer_size);

		if (i == 0)
		{
			//Yuv Chart
			memcpy(ptagImageData->pImageData, vYuvChartBuffer, ptagImageData->tFileInfo.nFileSize);			//ImageEncode
		}
		else if (i == 1)
		{
			//Raw Chart
			memcpy(ptagImageData->pImageData, vRawChartBuffer, ptagImageData->tFileInfo.nFileSize);		//ImageEncode
		}
		else
		{
			//Raw Oc
			memcpy(ptagImageData->pImageData, vRawBuffer_3000k, ptagImageData->tFileInfo.nFileSize);
		}
		

		//pBuffer = Raw Image°¡ ÀúÀåµÇ¾îÀÖ´Â Buffer pointer
		//buffer_size = Raw Image Å©±â
		vImageDatas.push_back(ptagImageData);
	}
	...
	_stprintf_s(szFileName, SIZE_OF_1K, _T("%s\\%s_%02d%02d%02d_%s.jxl"), szPath, cnamePath, stSysTime.wHour, stSysTime.wMinute, stSysTime.wSecond, m_szModel);

	int nRet = ENC_JXL_SAVE_IMAGEp(szFileName, vImageDatas.data() , (int)vImageDatas.size()); // ÀúÀåÇÒ ÆÄÀÏ¸í
	
	for (int i = 0; i < vImageDatas.size(); i++)
	{
		IMAGE_DATA* ptagData = (IMAGE_DATA*)vImageDatas[i];
		if (ptagData) { delete ptagData; }
		ptagData = NULL;
	}
...
//
//-----------------------------------------------------------------------------
void CLaonGrabberWrapper::CopyInspImage(int nType, BYTE* GrabImage , bool sumImageSave)
{
	CString str;
	if (nType == OC_RAW_IMAGE)	//oo use
	{
		str.Format("OcRaw");
		memcpy(g_clLaonGrabberWrapper[m_nUnit].vRawBuffer_3000k, GrabImage, g_clLaonGrabberWrapper[m_nUnit].m_pBoard->GetFrameRawSize());
		if (g_clModelData[m_nUnit].m_nRawSavePass == 0)
		{
			RawImageSave(str, g_clLaonGrabberWrapper[m_nUnit].vRawBuffer_3000k, RAW_OC);
		}
	}
	else if (nType == CHART_RAW_IMAGE)
	{
		str.Format("ChartRaw");
		if (sumImageSave == false)
		{
			memcpy(g_clLaonGrabberWrapper[m_nUnit].vRawChartBuffer, GrabImage, g_clLaonGrabberWrapper[m_nUnit].m_pBoard->GetFrameRawSize());
		}
		if (g_clModelData[m_nUnit].m_nRawSavePass == 0)
		{
			RawImageSave(str, g_clLaonGrabberWrapper[m_nUnit].vRawChartBuffer, RAW_CHART);
		}
	}
	else if (nType == CHART_YUV_IMAGE)
	{
		str.Format("ChartYuv");
		memcpy(g_clLaonGrabberWrapper[m_nUnit].vYuvChartBuffer, GrabImage, g_clLaonGrabberWrapper[m_nUnit].m_pBoard->GetFrameRawSize());

		if (g_clModelData[m_nUnit].m_nRawSavePass == 0)
		{
			RawImageSave(str, g_clLaonGrabberWrapper[m_nUnit].vYuvChartBuffer, YUV_CHART);
		}
	}
	else
	{
		//¸ðµÎ ÃÊ±âÈ­?
		memset(g_clLaonGrabberWrapper[m_nUnit].vRawChartBuffer, 0, N_RAW_SIZE);// m_pBoard->GetFrameRawSize());
		memset(g_clLaonGrabberWrapper[m_nUnit].vRawBuffer_3000k, 0, N_RAW_SIZE);//m_pBoard->GetFrameRawSize());


		memset(g_clLaonGrabberWrapper[m_nUnit].vYuvChartBuffer, 0, N_YUV_SIZE);// m_pBoard->GetFrameRawSize());


		_stprintf_s(strRawFileName[YUV_CHART], SIZE_OF_1K, _T("ChartYuv"));
		_stprintf_s(strRawFileName[RAW_CHART], SIZE_OF_1K, _T("ChartRaw"));
		_stprintf_s(strRawFileName[RAW_OC], SIZE_OF_1K, _T("OcRaw"));
...
void CLaonGrabberWrapper::RawImageSave(CString nType , BYTE* LoadImg, int index, bool mJpgSaveUse)
{
	TCHAR szPath[SIZE_OF_1K];
...
//#endif
	...
	int nSize = g_clLaonGrabberWrapper[m_nUnit].m_pBoard->GetFrameRawSize();
	savefile.Write(LoadImg, nSize);
